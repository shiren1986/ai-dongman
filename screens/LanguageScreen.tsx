    ...
    <div className="h-full flex flex-col bg-background-light dark:bg-background-dark">
      <nav className="sticky top-0 z-50 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center justify-between px-4 h-14">
          ...
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">{t('header_language')}</h2>
      ...
      <main className="flex-1 px-5 py-6 flex flex-col gap-3 overflow-y-auto no-scrollbar">
        {LANGUAGES.map((lang, idx) => (
          <label 
            key={idx}
            className={`group relative flex cursor-pointer items-center justify-between rounded-2xl p-4 shadow-sm border transition-all duration-200 active:scale-[0.99] ${language === lang.value ? 'bg-primary/5 border-primary ring-2 ring-primary/20' : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700'}`}
          >
            <div className="flex flex-col">
              <span className={`text-base font-bold transition-colors ${language === lang.value ? 'text-primary' : 'text-slate-900 dark:text-white'}`}>{lang.label}</span>
