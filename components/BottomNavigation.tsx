...
const BottomNavigation: React.FC<BottomNavigationProps> = ({ activeTab, onTabChange, language = 'zh-CN' }) => {
  ...
  return (
    <nav className="absolute bottom-0 left-0 right-0 z-50 bg-[#120a1f]/95 backdrop-blur-2xl border-t border-white/5 pb-10 pt-4">
      <div className="flex items-center justify-around px-6 h-24">
        <button 
          onClick={() => onTabChange(Tab.Library)}
          className={`flex flex-col items-center justify-center gap-3 flex-1 h-full transition-all active:scale-90 ${activeTab === Tab.Library ? 'text-primary' : 'text-slate-500'}`}
        >
        ...
        <button 
          ...
          className={`flex flex-col items-center justify-center gap-3 flex-1 h-full transition-all active:scale-90 ${activeTab === Tab.Assets ? 'text-primary' : 'text-slate-500'}`}
        >
        ...
        <button 
          ...
          className={`flex flex-col items-center justify-center gap-3 flex-1 h-full transition-all active:scale-90 ${activeTab === Tab.Settings ? 'text-primary' : 'text-slate-500'}`}
        >
